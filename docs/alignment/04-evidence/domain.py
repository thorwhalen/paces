...
with torch.no_grad():
    ...
    Q = [
        ...
        "a person dancing",
        "a person standing still talking to the camera",
        "a wide shot of a person in a room",
    ...
    T = model.get_text_features(**i).float().cpu()
    T = T / T.norm(dim=-1, keepdim=True)
S = (T @ F.T).numpy()
for q, row in zip(Q, S):
    print(f"  {row.mean():+.4f} mean  {row.max():+.4f} max   {q}")
print(
    f"\n  cartoon minus photograph, mean over 166 frames: {S[0].mean() - S[1].mean():+.4f}"
    f"  (cartoon wins on {(S[0] > S[1]).mean() * 100:.0f}% of frames)"
)
print(
    f"  'dancing' minus 'standing still talking': {S[2].mean() - S[3].mean():+.4f}"
