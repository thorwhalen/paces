...
DECOY = [
    "une personne qui fait frire des oignons dans une poêle",
...
def stats(S, name):
    ...
    z = (peak - mean) / std
    print(
        f"{name:8s} n={len(S):2d}  peak-cos {peak.min():.3f}..{peak.max():.3f} (mu {peak.mean():.3f})"
...
pt, zt = stats(St, "TRUE")
pd_, zd = stats(Sd, "DECOY")
print(
    f"\n  separation on raw peak cosine : AUC = {np.mean([(a > b) + 0.5 * (a == b) for a in pt for b in pd_]):.2f}"
)
print(
    f"  separation on peak z-score    : AUC = {np.mean([(a > b) + 0.5 * (a == b) for a in zt for b in zd]):.2f}"
)
...
pd2 = P[9:].max(1)
print(
    f"    TRUE peak {pt2.min():.3f}..{pt2.max():.3f} | DECOY peak {pd2.min():.3f}..{pd2.max():.3f}"
