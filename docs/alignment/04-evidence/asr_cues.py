...
dur = 36.24
print(
    f"ASR: {dt:.1f}s for {dur:.0f}s audio ({dur / dt:.1f}x realtime, mlx large-v3-turbo)"
)
segs = [(s["start"], s["end"], s["text"].strip()) for s in r["segments"]]
for s in segs:
    print(f"  [{s[0]:6.1f}-{s[1]:6.1f}] {s[2]}")
...
for name, pat in (("ORDINAL", ORDINAL), ("IMPERATIVE", IMPER), ("CLOSER", CLOSERS)):
    for m in re.finditer(pat, full, re.I):
        ...
        print(
            f"  {name:11s} t={t if t is None else round(t, 1):>6}  '{m.group(0).strip()}'"
        )
