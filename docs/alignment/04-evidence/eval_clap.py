...
GT=[(0,40,'music'),(40,76.2,'speech'),(76.2,82.2,'silence'),(82.2,122.2,'music')]
def label(t):
    for a,b,l in GT:
        if a<=t<b: return l
    return 'music'

x,sr=sf.read(f'{SP}/mix.wav'); x=x.astype(np.float32)
...
for i in range(len(x)//n):
    ...
    base.append((float(np.sqrt((w**2).mean())), float(X[(f>=30)&(f<=140)].sum()/X.sum())))
base=np.array(base); dt_base=time.time()-t0
...
for mid in ['laion/clap-htsat-unfused','laion/larger_clap_general']:
    ...
    for k in range(0,len(starts),16):
        ...
        with torch.no_grad(): f=model.get_audio_features(**ai)
        feats.append(f)
    F=torch.cat(feats); F=F/F.norm(dim=-1,keepdim=True); dt=time.time()-t0
    S=(Tt@F.T).numpy()
    pr=np.array([QS[i].split()[0] for i in S.argmax(0)])
    pr=np.array(['music' if p=='music' else ('speech' if p=='a' else 'silence') for p in pr])
    ...
    for l in ('music','speech','silence'):
        m=tr==l
        if m.sum(): print(f"   {l:8s} n={m.sum():3d} recall={(pr[m]==l).mean():.2f} "
