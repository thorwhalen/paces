...
model = AutoModel.from_pretrained(mid, dtype=torch.float16).to("mps").eval()
FR_ = sorted(glob.glob(f"{SP}/frames/*.jpg"))
T = np.arange(len(FR_)) + 0.5
...
dt = time.time() - t0
print(
    f"so400m-384: {len(imgs)} frames in {dt:.1f}s = {len(imgs) / dt:.1f} fps, dim={F.shape[1]}"
)
for tag, txt in (("FR", FR), ("EN", EN)):
    S = (enc_txt(txt) @ F.T).numpy()
    h = [block_of(T[int(S[i].argmax())]) == i for i in range(9)]
    print(
        f"  full-frame argmax {tag}: {sum(h)}/9  peaks={[round(float(T[int(S[i].argmax())]), 0) for i in range(9)]}"
    )
...
def frames_of(c, every=5):
    ...
    while True:
        ...
        if not ok:
            break
        if k % every == 0:
            o.append(Image.fromarray(cv2.cvtColor(fr, cv2.COLOR_BGR2RGB)))
        k += 1
    cap.release()
    return o
...
for tag, txt in (("FR", FR), ("EN", EN)):
    ...
    rows = []
    for c, gt in CLIPS:
        s = (Tt @ enc_img(frames_of(c)).T).mean(1).numpy()
        o = list(np.argsort(-s))
        r = o.index(gt)
        t1 += r == 0
        t3 += r < 3
        rows.append(f"{c}:r{r + 1}")
        m = s[o[0]] - s[o[1]]
