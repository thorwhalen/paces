...
def ff(c):
    cap=cv2.VideoCapture(f'{MED}/{c}.mp4');ok,fr=cap.read();cap.release()
...
for T0 in (50.9,51.2,59.4,59.8):
    ...
    for c,s in RT.items():
        t=int(round(s-T0)); 
        if not (0<=t<len(frames)): row.append(f"{c}:oob"); continue
        tpl0=ff(c); best=-2
        for w in range(110,260,10):
            ...
            for tt in (t-1,t,t+1):
                if 0<=tt<len(frames):
                    best=max(best,float(cv2.matchTemplate(frames[tt],tpl,cv2.TM_CCOEFF_NORMED).max()))
        row.append(f"{c}:{best:.2f}@{t}s"); tot+=best
