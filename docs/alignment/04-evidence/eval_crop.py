...
CLIPS=[('b2',1),('b3',2),('b4a',3),('b4b',3),('b7',6),('b9',8)]   # 0-based block index

def frames_of(c, every=5):
    cap=cv2.VideoCapture(f'{MED}/{c}.mp4'); out=[]; k=0
    while True:
        ok,fr=cap.read()
        if not ok: break
        if k%every==0: out.append(Image.fromarray(cv2.cvtColor(fr,cv2.COLOR_BGR2RGB)))
        k+=1
...
def enc_img(ims):
    with torch.no_grad():
        i=proc(images=ims,return_tensors='pt').to('mps'); f=model.get_image_features(**i).float().cpu()
...
for tag,texts in (('FR',FR),('EN',EN)):
    ...
    for c,gt in CLIPS:
        ...
        r=order.index(gt)
        top1+= r==0; top3+= r<3
        print(f"   {c:4s} true=blk{gt+1}  rank={r+1}  top1=blk{order[0]+1}  margin={s[order[0]]-s[order[1]]:+.4f}")
    print(f"   top-1 {top1}/{len(CLIPS)}   top-3 {top3}/{len(CLIPS)}   (chance top-1 = 0.67/6)")
