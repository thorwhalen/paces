...
frames = [cv2.cvtColor(cv2.imread(p), cv2.COLOR_BGR2GRAY) for p in sorted(glob.glob(f'{SP}/frames/*.jpg'))]
# frames are 400x225 (scaled from 854x480), 1 fps, frame i covers [i, i+1)
print('frame shape', frames[0].shape)

def first_frame(clip):
    cap = cv2.VideoCapture(f'{MED}/{clip}.mp4'); ok, fr = cap.read(); cap.release()
...
for clip, src_start in RT.items():
    ...
    for w in range(120, 260, 10):      # template width in the 400px-wide frame space
        ...
        if h >= frames[0].shape[0]: continue
        tpl = cv2.resize(tpl0, (w,h), interpolation=cv2.INTER_AREA)
        for i, f in enumerate(frames):
            r = cv2.matchTemplate(f, tpl, cv2.TM_CCOEFF_NORMED)
            v = float(r.max())
            if v > best[0]: best=(v, i, w)
    out[clip] = dict(score=best[0], filage_t=best[1], tpl_w=best[2], src_start=src_start,
                     implied_T0=round(src_start-best[1],2))
    print(f"{clip:5s} score={best[0]:.3f} filage_t={best[1]:4d}s tplw={best[2]:3d} "
