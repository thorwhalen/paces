...
exec(
    open(
        "/private/tmp/claude-501/-Users-thorwhalen-Downloads/0f75703c-6761-4aa0-b796-aafe02c94155/scratchpad/eval_vlm.py"
    )
    .read()
    .split("FRAMES = sorted")[0]
)
...
def hits(S, name):
    ...
    print(
        f"    {name:34s} {sum(h)}/9   peaks={[round(float(T[int(S[i].argmax())]), 0) for i in range(9)]}"
    )
    return sum(h)
...
for tag in ("FR", "EN"):
    ...
    P = np.exp(S * 100)
    P = P / P.sum(0, keepdims=True)  # softmax over queries per frame
