...
FR = ["une personne debout immobile qui se met en place",
...
EN = ["a person standing still getting into position",
...
def run(model_id, texts, tag, device='mps'):
    ...
    with torch.no_grad():
        ...
        kw = dict(padding='max_length', max_length=64) if 'siglip' in model_id else dict(padding=True)
        ti = proc(text=texts, return_tensors='pt', truncation=True, **kw).to(device)
        Tt = model.get_text_features(**ti).float().cpu(); Tt=Tt/Tt.norm(dim=-1,keepdim=True)
    S = (Tt @ F.T).numpy()            # (9 blocks, 166 frames)
    dt=time.time()-t0
    # -- argmax per block, independently
    hits=0; rows=[]
    for i in range(9):
        j=int(S[i].argmax()); t=T[j]; b=block_of(t); ok = (b==i)
        hits+=ok
        rows.append(f"  blk{i+1}: peak@{t:6.1f}s -> block{b+1} {'HIT ' if ok else 'miss'} "
    ...
    D = np.full((n,m), -1e9); P=np.zeros((n,m),int)
    D[0,0]=Z[0,0]
    for j in range(1,m): D[0,j]=D[0,j-1]+Z[0,j]; P[0,j]=0
    for i in range(1,n):
        for j in range(i,m):
            stay = D[i,j-1] if D[i,j-1]>-1e8 else -1e9
            new  = D[i-1,j-1]
            if new>=stay: D[i,j]=new+Z[i,j]; P[i,j]=1
            else: D[i,j]=stay+Z[i,j]; P[i,j]=0
    # backtrace
    bounds=[m]; i,j=n-1,m-1
    while i>0:
        while j>0 and P[i,j]==0: j-=1
        bounds.append(j); i-=1; j-=1
    ...
    print(f"  DP boundary abs err (s): median={np.median(dp_err):.1f} mean={np.mean(dp_err):.1f} max={max(dp_err):.1f}")
    return S
