...
for c in clips:
    ...
    S._cut(
        ...
        c["start"],
        c["dur"],
        f"crop={cw}:{ch}:{cx}:{cy},fps=25,scale=560:-2",
        tmp,
    )
    n = S.stylize(models, tmp, out, flat_bg=True, crf="26")
    os.remove(tmp)
    print(
        f"{out} {n}f {os.path.getsize(out) / 1024:.0f}KB  [{time.time() - t0:.0f}s]",
        flush=True,
    )
...
print(
    f"{out} {n}f {os.path.getsize(out) / 1e6:.2f}MB  [{time.time() - t0:.0f}s]",
    flush=True,
)

mp4 = sum(
    os.path.getsize(f"{OUT}/{f}")
    for f in os.listdir(OUT)
    if f.endswith((".mp4", ".jpg"))
)
gif = sum(os.path.getsize(f"{OUT}/{f}") for f in os.listdir(OUT) if f.endswith(".gif"))
print(
    f"DONE  page media {mp4 / 1e6:.2f} MB · gifs {gif / 1e6:.2f} MB  [{time.time() - t0:.0f}s]"
)
