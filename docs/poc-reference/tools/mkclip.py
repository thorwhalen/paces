...
def _person_boxes(start, dur, probe_fps=5):
    ...
    for i in range(0, len(frames), 8):
        for r in _MODEL.predict(
            frames[i : i + 8], classes=[0], conf=0.3, imgsz=768, verbose=False
        ):
...
def crop_box(start, dur, aspect=0.8, pad=0.18, lo=4, hi=96):
    ...
    if len(boxes) == 0:
        return 0, 0, W, H
    x0 = np.percentile(boxes[:, 0], lo)
    y0 = np.percentile(boxes[:, 1], lo)
    x1 = np.percentile(boxes[:, 2], hi)
    y1 = np.percentile(boxes[:, 3], hi)
    bw, bh = x1 - x0, y1 - y0
...
def main():
    ...
    w = a.width
    h = int(round(w / a.aspect)) if not a.crop else int(round(w * ch / cw))
    w -= w % 2
    h -= h % 2
    ...
    if fmt == "gif":
        ...
        subprocess.run(
            [
                ...
                SRC,
                "-vf",
                vf + f",palettegen=max_colors={a.colors}:stats_mode=diff",
                pal,
                "-loglevel",
                "error",
            ],
