...
subprocess.run(['ffmpeg','-y','-ss',str(start),'-to',str(end),'-i',SRC,
                '-vf',f'crop={w}:{h}:{x}:{y},fps={1/step},scale=400:-1','-q:v','3',
...
for i,f in enumerate(files):
    t = start + i*step
    im = Image.open(f); px,py = (i%cols)*W, (i//cols)*H
    sheet.paste(im,(px,py)); d.rectangle([px+2,py+2,px+104,py+32], fill='black')
